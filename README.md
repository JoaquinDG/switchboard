...
```bash
PYTHONPATH=src python3 -m unittest discover -s tests   # 337 tests
...
Every `model_id` in a catalog is a claim that a string will be accepted by a vendor, and **nothing in the offline suite can check it** — `MockProvider` answers to any id you give it. A typo, a renamed model, or an id copied from a pricing page that uses display names rather than API names survives all 337 tests and then fails as a hard 404 on first real traffic. This lists what each key can actually reach, diffs it against the catalog, and suggests close matches for anything missing. It only issues GETs to the models endpoints, so it generates no tokens.
...
Then: budget-aware policies, multi-auditor consensus, shadow routing for policy A/B, prompt-cache-aware costing, async providers, hard capability flags, and a counterfactual "why not X?" explainer.
...
tests/               337 tests (router, gates, triage, auditor, costs, resilience, catalog, live wiring)
evals/               9 routing scenarios, 40 triage prompts, 24 planner cases,
                     catalog_feedback (measured capability from traces),
                     latency_report (measured p50/p95 latency from traces)
