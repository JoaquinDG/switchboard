...
```bash
PYTHONPATH=src python3 -m unittest discover -s tests   # 345 tests
...
- **Per-token price alone understates some models' cost.** Claude 4.7 and later use a newer tokenizer that produces roughly 30% more tokens for the same text, so `claude-opus-5`, `claude-fable-5`, and `claude-sonnet-5` carry `token_multiplier: 1.3` — Anthropic's own rough figure, not something this repo measured. `router.estimate_cost` scales a task's *estimated* token counts by it before pricing; `actual_cost` is untouched, since a real invoice already reflects whichever tokenizer ran and the multiplier would double-count it there.
...
Every `model_id` in a catalog is a claim that a string will be accepted by a vendor, and **nothing in the offline suite can check it** — `MockProvider` answers to any id you give it. A typo, a renamed model, or an id copied from a pricing page that uses display names rather than API names survives all 345 tests and then fails as a hard 404 on first real traffic. This lists what each key can actually reach, diffs it against the catalog, and suggests close matches for anything missing. It only issues GETs to the models endpoints, so it generates no tokens.
...
- **A verified step can be worthless downstream** — a plan step can pass its own audit while only restating its input, and nothing today stops the plan from paying for the next one anyway.
...
tests/               345 tests (router, gates, triage, auditor, costs, resilience, catalog, live wiring)
