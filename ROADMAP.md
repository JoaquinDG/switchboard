...
### 3. Tokenizer-aware cost comparison — **DONE**
- [x] **Why it matters:** `examples/starter_catalog.json` already documents that Claude 4.7 and later use a tokenizer producing roughly 30% more tokens for the same text. Per-token price therefore understates their real cost, and the router compares vendors on price alone. Cross-vendor cost comparison is currently biased.
...
- **Shipped as:** `ModelSpec.token_multiplier` (default 1.0, validated positive) in `registry.py`; `router.estimate_cost` scales `task.est_input_tokens`/`task.est_output_tokens` by it before pricing, `router.actual_cost` is untouched. `examples/starter_catalog.json` sets `token_multiplier: 1.3` — Anthropic's own rough "30% more" figure, not a measurement — on `claude-opus-5`, `claude-fable-5`, and `claude-sonnet-5` (Claude 5-generation, i.e. 4.7+); `claude-haiku-4-5-20251001` predates 4.7 and is left at the default. Verified: `tests/test_router.py::TokenMultiplierCostTests` (estimate scales, actual doesn't, and two equally-priced-per-token models rank differently under `cost_first` once the multiplier is set) and `tests/test_registry.py` (default, validation, catalog loading).
