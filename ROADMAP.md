...
- **Known limit:** confidence gating is bounded by the heuristic's self-knowledge. One unmarked compound scored 0.20 (gate opens, model rescues it); the other scored 0.95 while being wrong, so the gate never opened. **Update:** the specific vocabulary gap behind the second case is fixed — see item 1c below. Confidence gating remains bounded by whatever the heuristic's vocabulary does not yet cover; a learned confidence would close that more generally.
...
### 1c. The planner's confidence has blind spots — **DONE**
- [x] **Why it mattered:** the model gate can only fire where the heuristic reports low confidence, and measured on unmarked compound requests it was right about its own uncertainty half the time. "Read this contract, extract the payment terms, and tell me if we should sign it" scored 0.20 and the gate rescued it. "Take these support tickets, work out which themes are growing, write me something I can send" scored **0.95 while being wrong**: no connective, and the phrasings matched none of the work-verb patterns, so it saw one job and was sure of it. A blind spot is worse than a known unknown.
- **Root cause:** `_WORK_VERBS["creative"]` required an article before its object noun (`write (?:the |a )?(?:copy|post|...)`, no match on "write me something"), and no pattern covered "work out" / "figure out" at all — so the second clause's judgement verb and the third clause's drafting verb were both invisible to `_distinct_work_kinds`, and the request looked like one job.
- **Fix:** widened `_WORK_VERBS["creative"]` to accept `me`/`us` before the object noun and added `something` to the object list; widened `_WORK_VERBS["reasoning"]` to include `work(?:ed)? out` / `figure out`. No other pattern changed.
- **Done looks like:** the unmarked-compound gate-fire rate in `planner_eval.py` improves without the false-split rate moving off 0. **Measured:** gate-fire rate on `COMPOUND_UNMARKED` went from 1/2 to 2/2; `NOT_COMPOUND` false-split rate stayed 0/10 (verified by hand against all ten negatives — none contain "work out", "figure out", "write me", or "write us").
- **Trap avoided:** the fix does not touch the global confidence values (0.20 / 0.80 / 0.95 / enumeration-ordinal-sequence scores) or the threshold — it only widens which requests get correctly recognised as naming two kinds of work. A request that genuinely names one kind of work still scores 0.95; the gate does not open more broadly, only more correctly on requests that were always compound.
