...
- **Shipped as:** a third field on the audit verdict, `adds_value: true|false|null`, parsed the same tolerant/fail-open way as `pass`/`score` except that an unreadable answer falls back to `null` ("unknown") rather than `false` — this field only ever *suppresses* spend, so it must not manufacture a suppression out of a parsing failure. The prompt asks for it only in relation to a labelled `--- OUTPUT OF STEP ... ---` section in the audited prompt, i.e. only when the step actually had a prior step's output as input; a step with nothing upstream reports `null`, never `false`. `Broker.run_plan` treats `adds_value=False` on a step with real injected context the same way it treats a failed audit for the *purpose of halting dependents only*: the step's own `result.verified` is untouched (it did pass), but steps depending on it are skipped, with the skip reason naming which case applied ("did not verify" vs "passed audit but added no value over its own input"). Governed by the existing `plan_halt_dependents_on_failure` toggle — no new policy knob. `AuditVerdict.adds_value`, `Attempt.audit_adds_value`, `BrokerResult.adds_value`, `StepResult.adds_value` and the replayed equivalent are all threaded through so the trace and `replay.py` can reconstruct why a step was cut off. Tests in `tests/test_auditor.py` (parsing, including the fail-open cases) and `tests/test_replay.py::NoAddedValueTests` (a restating step still verifies, still stops its dependents, and a step with no upstream input is never flagged).
- **Known limit:** this only catches restatement *after* the fact — the first refusal in a chain cannot be pre-empted, since there is nothing to judge it against yet. What it stops is the *second* and later restatements in a chain, which is where the measured $0.02531 was actually spent.
