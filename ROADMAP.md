...
### 2. Context-window awareness in routing — **DONE**
- [x] **Why it mattered:** `ModelSpec.context_window` was stored, validated, and **never read by the router** (verified: no reference outside `registry.py`). A task with 400k estimated input tokens could be routed to a 200k model, which fails at the provider with a hard error rather than being caught as a routing constraint.
- **Shipped as:** `router.fits_context(task, spec)` — a hard gate run in `route()` *before* the frontier gate (context is capacity, not quality; a frontier model too small for the task must not survive purely because the frontier gate would otherwise force frontier tier). Excluded models are named in `RoutingDecision.warnings` and folded into the rationale string, and a new `"context window gate applied"` entry appears in `.gates`. `Broker._escalation_target` applies the same check, so a failed audit cannot escalate into a model that would hard-fail at the provider on context alone — it degrades to the next tier up exactly the way an exhausted tier already did for "no untried candidates".
- **Trap, avoided:** this is a *gate*, not a score. `fits_context` never appears in `score_models`; a model either clears it or is not a candidate.
- **Second trap, avoided:** unlike the frontier and qualification gates, there is no honest "degrade upward" for capacity — a bigger tier is not necessarily a bigger context window. When nothing in the registry can hold the task, `route()` raises `ContextWindowExceededError` naming the shortfall rather than silently handing the task to a model that will fail at the provider.
- **Verified:** `tests/test_router.py::ContextWindowGateTests` (exclusion, rationale naming, gate ordering ahead of the frontier gate, the raise path, and that a small task never trips the gate) and `tests/test_broker.py::EscalationTests::test_escalation_skips_a_model_that_cannot_hold_the_task`.
- **Not done:** `max_completion_tokens` (the output-side ceiling the same public index reports) is not enforced. `ModelSpec` has no field for it yet — adding one is real catalog work, not a one-line addition to this gate — so it is left for a follow-up rather than bundled in here.
