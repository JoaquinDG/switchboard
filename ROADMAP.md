...
### 7. Multi-auditor consensus for high-stakes tasks — **DONE**
- [x] **Why it matters:** One auditor is one opinion. For high-complexity or explicitly flagged work, N independent auditors and a majority verdict is meaningfully stronger evidence — and the cross-lab machinery to pick genuinely independent auditors already exists.
...
- **Shipped as:** `Policy.multi_auditor_count` (default 1, off) and `Policy.multi_auditor_complexity_gate` (default `None`) alongside `Task.high_stakes` — a panel fires only when the policy asks for more than one auditor *and* the task is either flagged or at/above the gate. `auditor.py` gained `pick_auditors` (spreads seats across distinct labs before repeating one, reusing the same `most_capable`/`cheapest_qualified` ranking `pick_auditor` already used) and `audit_consensus`, returning a `ConsensusVerdict`: `passed` is a strict majority (a tie fails closed), `unanimous` is a separate field, and `.issues` leads with `"audit panel split N-M (...)"` before each auditor's own findings, attributed by name — so a 2-1 split is never just a mean score. `Broker.run` records the full per-auditor breakdown on `Attempt.consensus_verdicts` and `Attempt.audit_unanimous` (both `None` outside a panel), and `_trace` mirrors `audit_unanimous` at the top level as `final_audit_unanimous`, next to the existing `final_audit_cross_lab`. No new cost field: `ConsensusVerdict.cost_usd` sums every seat and flows through the same `audit_cost_usd` a single auditor already reported through.
